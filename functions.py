...
def save_game(winner, hero_name="", num_stars=0):
    try:
        # Read current monster kills count
        try:
            with open("save.txt", "r") as file:
                lines = file.readlines()
                if lines:
                    monster_kills = int(lines[0].strip())
                else:
                    monster_kills = 0
        except FileNotFoundError:
            monster_kills = 0

        # Update monster kills if hero won
        if winner == "Hero":
            monster_kills += 1

        # Write updated monster kills and game result
        with open("save.txt", "w") as file:
            file.write(f"{monster_kills}\n")
            if winner == "Hero":
